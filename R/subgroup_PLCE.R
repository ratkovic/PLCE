...
sparsereg <- function(y,
  ...
  if (alpha.prior == "parametric")
    alpha <- alpha0
  if (alpha.prior == "balanced")
    alpha <- alpha2
  #if(!EM) alpha<-n*log(p)
  
  if (alpha.prior == "custom")
    alpha <- alpha.use
  ...
  mcmc.obj.init <-
    matrix(NA, nrow = floor(iter / thin), ncol = ncol(X))
  colnames(mcmc.obj.init) <- colnames(as.matrix(X))
  mcmc.obj.init <-
    as.mcmc(mcmc.obj.init,
            start = burnin,
            end = burnin + iter,
            thin = thin)
  ...
  gammaaccept.run <-
    gamma.run <-
    lambda.run <- sigma.sq.run <- rep(NA, floor(iter / thin))
  ...
  if (length(beta.init) != ncol(X)) {
    ...
    beta <-
      beta.last  <-
      beta.init <-
      as.vector(solve_cpp(crossprod(X) + priormat, crossprod(X, y))$beta)
    up.wts <- matrix(1, nrow = 3, ncol = length(beta))
    ...
    if (sum(is.na(beta)) > 0)
      beta[is.na(beta)] <-
      beta.last[is.na(beta)] <- rnorm(sum(is.na(beta.last))) / 5
    if (sum(beta == 0) > 0)
      beta[beta == 0] <-
      beta.last[beta == 0] <- rnorm(sum(beta == 0)) / 5
  ...
  prec <-
    ifelse(type == "linear", 1 / sd_cpp(as.vector(y - X %*% beta)) ^ 2, 1)
  sigma.sq <- 1 / prec
  ...
  for (i.iter in 1:floor((burnin + iter) / thin)) {
    for (i.thin in 1:thin) {
      ...
      if (sparseregweights) {
        ...
        wts[2:(p + 1)] <-
          apply(wts.mat, 1, updatewts.EM, seq.obj0 = seq.obj)
      ...
      if (i.iter < 10)
        wts <- rcppClamp(wts, 1e-5, 1e3)#pmax(pmin(wts, 1e3), 1e-5)
      beta.trim <- rcppClamp(abs(beta), 1e-5, 1e10)
      Ewtsqtausq <-
        (beta.trim[1:(p + 1)] / lambda * sqrt(prec) * wts[1:(p + 1)]  + 1 / lambda.sq)
      Etausqinv <-
        (lambda / beta.trim[1:(p + 1)]  * sqrt(sigma.sq) * wts[1:(p + 1)])
      if (length(beta) > 1) {
        Dtau <- (1 / Etausqinv)
        Dtau[Dtau > 1e5] <- 1e5
        Dtau[1] <- 1e5
        if (i.iter < 4)
          Dtau <- rcppClamp(Dtau, 1e-3, 1e3)
      } else {
        Dtau <- as.matrix(1e5)
      }
      ...
      if (length(beta) != length(beta2.last))
        beta <- beta2.last
      if (sum(!is.finite(beta)) > 0)
        beta <- beta2.last
      ...
      if (i.iter < 10)
        beta <-
        sign(beta) * rcppClamp(abs(beta), 1e-5, 1e10)#pmax(abs(beta), 1e-5)
      
      # print(max(abs(beta2.last - beta2)) )
      # print(usesparseregweights)
      # print(conv.res)
      
      tryCatch(if (max(abs(beta2.last - beta2)) < tol &
      ...
      prec <- ((n / 2 + p / 2 + 1) /
                 (sum((
                   y - fits.curr - fits.res
                 ) ^ 2) + sum(beta[2:(p + 1)] ^ 2 / (Dtau[2:(p + 1)])) / 2))
    ...
    if (i.iter > ceiling(burnin / thin)) {
      if (EM & conv) {
        ...
        fits.run <-
          matrix(link(as.vector(X0 %*% beta) + fits.res), ncol = 1)
        ...
        if (edf)  {
          if (sum(beta != 0) > 1) {
            ...
            var.inv <- ginv(XprimeXdf + Dtauinv.df)
            svd.inv <- svd(var.inv)
            svd.inv$d <- sqrt(svd.inv$d)
            keeps <-
              (cumsum(svd.inv$d) / sum(svd.inv$d)) < 0.9#svd.inv$d/svd.inv$d[1]>1e-4
            if (sum(keeps) <= 1) {
              chol.inv <- NULL
              chol.adj <- NULL
            } else{
              ...
              chol.inv <-
                svd.inv$u[, keeps] %*% (diag(diag.svd) %*% t(svd.inv$v[, keeps]))
              #svd2 <- svd(X[,beta.temp!=0]%*%chol.inv)
              #chol.inv<- svd2$v[,svd2$d>0]
            }
            dof <- sum(diag(XprimeXdf %*% var.inv))
          }
          else {
            chol.inv <- diag(1)
            dof <- 1
          ...
          GCV <-
            (mean((y0 - fits.curr - fits.res) ^ 2) / (1 - dof / n * (log(n) / 2)) ^
               2) ^ .5
